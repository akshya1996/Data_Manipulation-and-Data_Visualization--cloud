...
def upload():
	...
	sql="""LOAD DATA LOCAL INFILE %s INTO TABLE earthquakes FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '"' LINES TERMINATED BY '\n' IGNORE 1 LINES"""
	cursor=db.cursor()
...
def memcache_page():
	...
	for i in range(1,30):
		...
		#query="select nst from earthquakes limit {}".format(number)
		query="select * from earthquakes where (latitude between %s and %s) and (longitude between %s and %s)".format(number)
...
def latlon():
	...
	fromlon=str(fromlon)
	tolon=str(tolon)
	sql="select * from earthquakes where (latitude between %s and %s) and (longitude between %s and %s)"
	cursor=db.cursor()
...
def addnum():
	...
	num1=str(num1)
	num2=str(num2)
	sql="insert into numdata (number1,number2) values (%s, %s)"
	cursor=db.cursor()
...
def magrange():
	...
	while fmag<=tmag:
		...
		fmagnitude=str(fmag)
		before=time.time()
		query1="select count(*) from earthquakes where mag =%s" 
		sql="select place from earthquakes where mag =%s limit 1 "
		cursor=db.cursor()
...
def latrange():
	...
	sql="select * from earthquakes where latitude between %s and %s order by mag desc limit 5"
	cursor=db.cursor()
...
def memcache_latrange():
	...
	query="select * from earthquakes where latitude"
	cursor=db.cursor()
	result=cursor.execute(query)
	count=result
	before=time.time()
	for i in range(1,30):
		number=random.randrange(0,5)
		#query="select nst from earthquakes limit {}".format(number)
		query="select * from earthquakes where latitude between %s and %s order by mag desc limit 5".format(number)
		enctext=hashlib.md5(query.encode())
		enctext=enctext.hexdigest()
		value=memc.get(enctext)
		if not value:
...
def placepattern():
	...
	sql="select * from earthquakes where place like '%"+location+"%'"
	cursor=db.cursor()
	cursor.execute(sql)
...
def memcache_findplace():
	...
	for i in range(1,30):
		number=random.randrange(0,5)
		#query="select nst from earthquakes limit {}".format(number)
		query="select * from earthquakes where place like '%"+location+"%'".format(number)
		enctext=hashlib.md5(query.encode())
		enctext=enctext.hexdigest()
		value=memc.get(enctext)
		if not value:
...
def locsource():
	location=request.form['place']
	sql="select * from earthquakes where locsource like '%"+location+"%'"
	cursor=db.cursor()
	cursor.execute(sql)
...
def memcache_locsource():
	...
	for i in range(1,30):
		number=random.randrange(0,5)
		query="select * from earthquakes where locsource like '%"+location+"%'".format(number)
		enctext=hashlib.md5(query.encode())
		enctext=enctext.hexdigest()
		value=memc.get(enctext)
		if not value:
...
def memcache_thousand():
	...
	while count<1000:
		...
		for i in range(1,30):
			number=random.randrange(0,5)
			query="select * from earthquakes where mag = %s".format(number)
			enctext=hashlib.md5(query.encode())
			enctext=enctext.hexdigest()
			value=memc.get(enctext)
			if not value:
...
def nearlocation():
	...
	cursor=db.cursor()
	sql="select * from earthquakes where place like '%"+location+"%' and place like '"+distance+"%'"
	cursor.execute(sql)
	data=cursor.fetchall()
	count=0
	for row in data:
...
def morequakes():
	...
	cursor=db.cursor()
	sql="select * from earthquakes where (etime like '%%T20%%' or etime like '%%T21%%' or etime like '%%T22%%' or etime like '%%T23%%' or etime like '%%T24%%' or etime like '%%T01%%' or etime like '%%T02%%' or etime like '%%T03%%' or etime like '%%T04%%' or etime like '%%T05%%') and (mag > %s)"
	cursor.execute(sql,(mag,))
	data=cursor.fetchall()
	count4=0
	for row in data:
		count4+=1
	query1="select * from earthquakes where mag>%s"
